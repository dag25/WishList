...
export const createWishlist = async pageLogin => {
	...
	const fullname =
		user.name || user.surname ? `${user.name || ''} ${user.surname || ''}`.trim() : user.login;
	...
	if (user.birthdate) {
		...
		const dayAndMonth = birthday.toLocaleDateString('default', {
			month: 'long',
			day: 'numeric',
		});
	...
	if (login === pageLogin) {
		const editBtn = createElement('button', {
			...
			innerHTML: `
				<svg class="profile__edit-icon" width="38" height="38" viewBox="0 0 38 38" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
					<path
